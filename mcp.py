...
def dateplt1(t,y,add=None,yl=5,fxlim=None,fylim=None,fighold=False,nomth=True,**kwargs):
...
def dateplt_m(t,y,yl=5,fxlim=None,fylim=None,ticksize=10,nomth=True,fmt='+-',**kwargs):
	from matplotlib.dates import YearLocator,DateFormatter,MonthLocator
	xchange = 0
	ychange = 0
	fig = plt.gcf()
...
def dateplt_m2(t,y,yl=5,fxlim=None,fylim=None,ticksize=10,nomth=True,fmt='+-',**kwargs):
...
def ocvshade(clon,clat,valsin,proj='cyl',lon0=215,res='c',blat=30,fz=(11.6,5.8),add=None,cbticksize=12,cbticks=None,nocb=None,vlo=None,vhi=None,lat0=None,wid=12000000,ht=8000000,land1=None,land2='blank',landmask=None,landdark=None,hatchmask=None,hatchalpha=None,**kwargs):
	...
	for ii in xrange(clonc1.shape[0]):
		...
		if ((clonc1[ii] > 350).any() & (clonc1[ii] < 50).any()):
			...
			lhs_x[final_size] = np.where(clonc1[ii] > 50,35.,clonc1[ii]).reshape(1,-1)
	...
	if (hatchmask is not None):
		...
		if hatchalpha is None:
			hatchalpha = 0.5
		p2 = PatchCollection(patches, edgecolors='none',facecolors='dimgrey', linewidths=0,zorder=2,alpha=hatchalpha)
		p2.set_array(hatch2)
	...
	if (landmask is not None):
		if landdark is not None:
			...
			nls2 = mpl.colors.BoundaryNorm(linspace(0.,1.,11), 180)
		p3 = PatchCollection(patches, edgecolors='none', linewidths=0,cmap=cm.Greys,norm=nls2)
		p3.set_array(land2)
