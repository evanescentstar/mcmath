...
import cftime
from cftime._cftime import datetime as cfdt
from cftime import num2date as cfn2d
from cftime import date2num as cfd2n
...
def blockave(x, blk, ax=0, weights=None):
    ...
    s1 = np.ndarray(xshp.shape, dtype=object)
    s1[:] = slice(None)

    for i in range(out.shape[ax]):
        s1[ax] = slice(i * blk, i * blk + blk)
...
>>>>>>> Stashed changes
    ...
    if N % 2 == 0:
        raise MCMathError("Window must have odd width.")
    X = x.size
    new_arr_len = X - 2 * np.int16(N / 2)
    new_arr_len = np.int16(new_arr_len)
    new_ind = np.zeros(new_arr_len, dtype=np.int16)
    for i in range(new_arr_len):
        index = i + N / 2
        new_ind[i] = index
...
>>>>>>> Stashed changes
    ...
    if x.ndim > 1:
        xshp = np.array(x.shape)
        new_arr_len = xshp[0] - 2 * np.int16(N / 2)
        rsdshp = xshp[1:]  ## residual shape (minus first axis, i.e., axis 0)
        newshp = np.append(new_arr_len, rsdshp)  ## new shape for output array
        newshp = np.int32(newshp)
        out = np.ma.masked_all(newshp, dtype=np.float32)
        out.data[:] = 1e20
        for i in np.ndindex(tuple(rsdshp)):
            rsdind = [slice(None)]
            for xx in range(rsdshp.size):
                rsdind.append(i[xx])
            if x[tuple(rsdind)].mask.all() == True: continue
            if bool(dtrnd):
                ts1 = mpl.mlab.detrend_linear(x[tuple(rsdind)])
            else:
                ts1 = x[tuple(rsdind)]
            ind, out[tuple(rsdind)] = run_mean_win_mskd2(ts1, wind)
...
def extrema_find(x, kind='all', sd=None, t=None, dt=None):
    ...
    if (t is not None) and (dt is not None):
        if ('min' == kind):
            min_ind = extr_ind.copy()
            new_ext_ind = []
            while min_ind.size > 0:
                cur1 = np.where(x[min_ind] == x[min_ind].min())[0][0]
                new_ext_ind.append(min_ind[cur1])
                remain_ind = np.where((t[min_ind] > (t[min_ind][cur1] + dt)) | (t[min_ind] < (t[min_ind][cur1] - dt)))[
                    0]
                min_ind = min_ind[remain_ind]
            ...
            while max_ind.size > 0:
                ...
                remain_ind = np.where((t[max_ind] > (t[max_ind][cur1] + dt)) | (t[max_ind] < (t[max_ind][cur1] - dt)))[
                    0]
...
def trnd_calc(t, x, bs=None, pval=0.025, intts=None):
    if isinstance(t[0], dt) or isinstance(t[0], cfdt):
        t = d2n(t)
    if isinstance(x, np.ma.MaskedArray):
        if x.mask.all() == True:
            return None
        ...
        if fullstat is None:
            ...
            if intts is None:
                ...
                if intts.shape != xshp[1:]:
                    raise MCMathError(
                        'size of intts must be the same as the input array minus the first dimension (time axis)')
...
def run_trnd_map(t, x, tlen, interval=5):
    """Calculates a series of running trend magnitudes over a time interval
    of length 'tlen', with a skip of 'interval', and returns an output file
    with the magnitudes along the first axis.
    [Previously, to avoid highly repetitious overlap, the intervals
    ...
    # intvl = tlen / 4
    intvl = interval
    numtrnd = np.int32(((x.shape[0] - tlen) / intvl) + 1)

    outarr = np.ma.masked_all((numtrnd, x.shape[1], x.shape[2]))

    for i in range(numtrnd):
...
def my_dtrnd(x, t=None, no_demean=True):
    """My version of a linear detrend of 1-d or multidimensional data.
    This version, unlike the	Numpy / Scipy code, can detrend a time
    series with masked holes properly (i.e., no slopes on the order of
    1e13 or all NaNs), by the same method as trnd_calc(), which provides (t,x) data pairs for only those unmasked data points.

    Input:  x - data to detrend
            t - time axis, if data.t does not contain it
        no_demean = set to False (or anything except True) to remove means from output
    ...
    if len(xshp) > 1:
        xshp0 = xshp[0]
        xr = x.reshape(xshp0, -1)
        xshpa = xr.shape
        xshp1 = xr.shape[1]

        dtrnd = np.ma.masked_all((xshpa), dtype=x.dtype)

        for i in range(xshp1):
            if isinstance(x, np.ma.MaskedArray):
                if xr[:, i].mask.all() == True:
                    continue
                if xr[:, i].mask.any() == True:
                    x1 = xr[:, i].data[~xr[:, i].mask]
                    t1 = t[~xr[:, i].mask]
                else:
                    x1 = xr[:, i].data
                    t1 = t
            ...
            blah = trnd_calc(t1, x1)
            if blah is not None:
                if no_demean is not True:
                    trendline = blah[0] * t + blah[1]
                else:
                    t0 = t - t[0]
                    trendline = blah[0] * t0
                dtrnd[:, i] = xr[:, i] - trendline
        dtrnd = dtrnd.reshape(xshp)
    elif len(xshp) == 1:
        dtrnd = np.ma.masked_all((xshp), dtype=x.dtype)
        if isinstance(x, np.ma.MaskedArray):
            if x.mask.all() == True:
                raise MCMathError('no data or all data masked')
            if x.mask.any() == True:
                x1 = x.data[~x.mask]
                t1 = t[~x.mask]
            else:
        ...
        raise MCMathError('size of input array can not be zero')
    if no_demean is not True:
        dtrnd = dtrnd - dtrnd.mean(axis=0)
    return dtrnd
...
def gcdist(p1, p2):
    ...
    x2, y2 = p2
    d = 2. * np.arcsin(
        min(1., np.sqrt(sindg((y2 - y1) / 2.) ** 2 + cosdg(y1) * cosdg(y2) * sindg((x1 - x2) / 2.) ** 2)))
    gcdist = d * R_earth
...
def corrtest(x1, x2, mcintts=None, npintts1=None, pval=0.05):
    ...
    if npintts1 is None:
        npintts1 = plt.acorr(x1, normed=True, maxlags=szm1)[1][szm1:][
                   0: np.where(plt.acorr(x1, normed=True, maxlags=szm1)[1][szm1:] < 0)[0][0]].sum()
        npintts2 = plt.acorr(x2, normed=True, maxlags=szm1)[1][szm1:][
                   0: np.where(plt.acorr(x2, normed=True, maxlags=szm1)[1][szm1:] < 0)[0][0]].sum()
...
def mth_ann_mean(x, dtin=None):
    ...
    outshp = np.array(N)
    outshp[0] = N12
    ind12shp = outshp.copy()
    ind12shp[0] = 1
    xout = np.ma.masked_all(outshp)

    mths = np.array([31., 28., 31., 30., 31., 30., 31., 31., 30., 31., 30., 31.])
    mthshp = np.ones(outshp.shape)
    mthshp[0] = 12
    mthshp = np.int32(mthshp)
    mths = mths.reshape(mthshp)

    for i in range(N12):
        yrx = x[i * 12:(i + 1) * 12]
        yrx2 = yrx
        if isinstance(yrx, np.ma.MaskedArray):
            ind12 = np.ma.where(yrx.count(axis=0) < 12, np.ma.masked, 1.)
            yrx2 = yrx * ind12.reshape(ind12shp)
        xout[i] = (yrx2 * (mths / mths.sum())).sum(axis=0)
...
def d2n(date_in, units=None, cal='proleptic_gregorian'):
    if units is None:
        numout = cfd2n(date_in, 'days since 0001-01-01 00:00:00', cal)
    else:
        numout = cfd2n(date_in, units, cal)
...
def n2d(num_in, units=None, cal='proleptic_gregorian'):
    if units is None:
        dateout = cfn2d(num_in, 'days since 0001-01-01 00:00:00', cal)
    else:
        dateout = cfn2d(num_in, units, cal)

    if hasattr(dateout, 'size'):
        datenew = np.ndarray(shape=dateout.size, dtype=object)
        for i in range(datenew.size):
            dtout = dateout.flatten()
            datenew[i] = dt(dtout[i].year, dtout[i].month, dtout[i].day, dtout[i].hour,
                            dtout[i].minute, dtout[i].second)
        dateout = datenew.reshape(dateout.shape)
    else:
...
def std_analy(data, t=None, fac=None):
    ...
    the annual anomaly standard deviation, the 10-year running mean from annual
    anomalies standard deviation, and the trends.  Input data must be monthly
    and in (T,Y,X) axis-order.
...
#### correlation statistics functions

def Zr(rin):
    """Turn a correlation coefficient into a Z-statistic
    """
    Zr = 0.5*(np.log(1+rin) - np.log(1-rin))
    return Zr

def invZr(Zrin):
    """To turn a Z-statistic back into a correlation
    """
